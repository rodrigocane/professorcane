...
CREATE DATABASE IF NOT EXISTS jogo_cidades DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE jogo_cidades;
...
-- Atualizar estados com suas respectivas regiões
UPDATE `estado` SET `id_regiao` = 1 WHERE `uf` IN ('AC', 'AP', 'AM', 'PA', 'RO', 'RR', 'TO');
UPDATE `estado` SET `id_regiao` = 2 WHERE `uf` IN ('AL', 'BA', 'CE', 'MA', 'PB', 'PE', 'PI', 'RN', 'SE');
UPDATE `estado` SET `id_regiao` = 3 WHERE `uf` IN ('DF', 'GO', 'MT', 'MS');
UPDATE `estado` SET `id_regiao` = 4 WHERE `uf` IN ('ES', 'MG', 'RJ', 'SP');
UPDATE `estado` SET `id_regiao` = 5 WHERE `uf` IN ('PR', 'RS', 'SC');
